...
const Titan = () => {
  ...
  useEffect(()=>{
    ...
    fetch(
      ...
      .then((json) => {
        const data = json.find(data => data.title.rendered.includes('TITAN CEO'))
        const about = json.filter(data => data.title.rendered.includes('About section'))
          ...
          <section className='titan_about'>
            {aboutSection &&
              aboutSection.map((about, i) => 
                <TitanAbout key={i} data={about} className={i % 2 === 0 ? 'brown' : 'gray'}/>)
            }
          </section>
        </>
      }
